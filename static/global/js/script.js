	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-down':
				...
				break;
			case 'arrow-left':
				offset_left = $('#slide-2 .content-row').offset().left + 452;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
			case 'arrow-right':
				offset_left = $('#slide-2 .content-row').offset().left - 452;
				if ( offset_left < $('body').width() - $('#slide-2 .content-row').width() ) {
					offset_left = $('body').width() - $('#slide-2 .content-row').width();
				}
				break;
		}
		...
		if ( offset_left != false ) {
			if ( $('#slide-2 .content-row').width() != $('body').width() ) {
				$('#slide-2 .content-row').stop(false, false).animate({
					left: offset_left
				}, 1500, 'easeInOutQuart');
			}
		}
