...
# -*- coding: utf-8 -*-
# @Author: Hollay.Yan
# @Date:   2014-09-27 10:54:20
# @Last Modified by:   Hollay.Yan
# @Last Modified time: 2014-09-28 20:46:23
...
class WechatReply(object):
    ...
    def __init__(self, message=None, **kwargs):
        if 'fromuser' not in kwargs and isinstance(message, WechatMessage):
            kwargs['fromuser'] = message.touser
        if 'touser' not in kwargs and isinstance(message, WechatMessage):
            kwargs['touser'] = message.fromuser
...
class MusicReply(WechatReply):
    ...
    def __init__(self, message, title='', description='', music_url='', hq_music_url='', thumb_media_id=None):
        ...
        super(
            MusicReply, self).__init__(message=message, title=title, description=description,
...
class ArticleReply(WechatReply):
    ...
    def add_article(self, article):
        if len(self._articles) >= 10:
            raise AttributeError('图文数量不能大于10')
        else:
